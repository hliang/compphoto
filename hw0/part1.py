...
def interlace(evens, odds):
  ...
  totalrows = evens.shape[0] + odds.shape[0]
  totalcols = evens.shape[1]
  totalchannels = evens.shape[2]
  # create a new array of given shape and type, filled with zeros.
  outimg = np.zeros((totalrows, totalcols, totalchannels), dtype=np.uint8)
